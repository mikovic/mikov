...
public class MysqlImgDao implements ImgDao {
    ...
    @Override
    public int addImgToIdea(int ideaId, int userId, String pathImg) throws SQLException {
        Connection con = null;
        PreparedStatement pst = null;
        int imgId = 0;
        try {
            con = ConnectionDataBaseFactory.getConnection();
            String query = "SELECT  img_Id FROM images WHERE user_id= ? AND path_img= ?";
            ...
            if (rs.next()) {
                ...
                query = "INSERT INTO images(idea_id, user_id, path_img) VALUES(?,?,?)";
                pst = con.prepareStatement(query);
                pst.setInt(1, ideaId);
                pst.setInt(2, userId);
                ...
                if (i==1){
                    pst = con.prepareStatement("SELECT img_id FROM images WHERE  user_id= ? AND path_img = ? ");
    ...
    public boolean deleteImg(int imgId) {
        ...
        try {
            con = ConnectionDataBaseFactory.getConnection();
            String query = "DELETE  FROM images WHERE img_id= ?";
            pst = con.prepareStatement(query);
            pst.setInt(1, imgId);
    ...
    public int getCountImgIdea(int ideaId) {
        ...
        try {
            ...
            String sql = "SELECT  COUNT(idea_id) FROM images WHERE idea_id = '" + ideaId+"'";
